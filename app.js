...
 async function doSearch(q) {
  ...
  // 🔹 ONLY search selected versions
  const activeFiles = [];
  if (state.versionA) activeFiles.push(state.versionA);
  if (state.versionB && state.versionB !== state.versionA) {
    activeFiles.push(state.versionB);
  }

  if (!activeFiles.length) {
    searchInfo.textContent = "Select at least one version";
    return;
  }
  ...
  for (const r of matches.slice(0, 300)) {
    const div = document.createElement("div");
    div.className = "search-item";

    const versionLabel = r.file
      .replace("_bible.json", "")
      .toUpperCase();

    const snippet = esc(r.text).replace(
      re,
      m => `<span class="highlight">${m}</span>`
    );

    div.innerHTML = `
      <strong>${esc(r.book)} ${r.chapter}:${r.verseKey} — ${versionLabel}</strong>
