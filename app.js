/* Final production app.js - single-file replacement
   Drop into your project and reload the page. */
...
(() => {
  ...
  const normCache = {};        // filename -> normalized structure
  const searchIndexCache = {}; // filename -> search index array

  // ---------- UTIL ----------
  const esc = s => String(s || '').replaceAll('&','&amp;').replaceAll('<','&lt;').replaceAll('>','&gt;');
  const showNotice = (msg, ms = 1400) => { if(!notice) return; notice.textContent = msg; notice.style.display = 'block'; setTimeout(()=> notice.style.display = 'none', ms); };
  const isNumber = n => Number.isFinite(Number(n));

  // ---------- NORMALIZE (uniform JSON described by you) ----------
  ...
  async function fetchAndNormalize(file){
    ...
    try{
      const url = BASE + file;
      console.log('[BibleReader] fetching', url);
      const res = await fetch(url);
      if(!res.ok) { console.error('[BibleReader] fetch failed', url, res.status); showNotice('Failed to load ' + file); return null; }
      ...
      console.error('[BibleReader] fetchAndNormalize error', err);
  ...
  function buildSearchIndex(file, norm){
    ...
    (norm.books || []).forEach((b, bi) => {
      (b.chapters || []).forEach((ch, ci) => {
        (ch || []).forEach((v, vi) => {
          arr.push({ bookIndex: bi, chapterIndex: ci, verseIndex: vi, book: b.name, chapter: ci + 1, verseKey: v.key, text: v.text, low: (v.text || '').toLowerCase() });
        });
      });
    });
  ...
  function showView(view){
    ...
    if(view === 'search') setTimeout(()=> searchBox && searchBox.focus(), 120);
  ...
  async function populateBooksForA(file){
    ...
    if(!n){ homeBook.innerHTML = "<option value=''>Book</option>"; return; }
  ...
  function clampIndices(){
    ...
    const n = normCache[state.versionA];
    if(!n || !n.books || !n.books.length){ state.bookIndex = 0; state.chapterIndex = 0; return; }
    ...
    if(!book || !book.chapters || !book.chapters.length){ state.chapterIndex = 0; return; }
  ...
  function paragraphRenderer(container, text){
    ...
    parts.forEach(p => { const d = document.createElement('div'); d.className = 'para'; d.textContent = p; container.appendChild(d); });
  }
  ...
  function renderCombined(idx, chapA, chapB){
    ...
    const header = document.createElement('div'); header.className = 'verse-num'; header.textContent = `Verse ${verseNum}`; block.appendChild(header);
    const contA = document.createElement('div'); contA.className = 'verse-text'; paragraphRenderer(contA, va ? va.text : ''); block.appendChild(contA);
    if(state.versionB){ const contB = document.createElement('div'); contB.className='verse-secondary'; paragraphRenderer(contB, vb ? vb.text : ''); block.appendChild(contB); }
  ...
  function renderRead(){
    try{
      ...
      if(state.verseKey){
        ...
        if(m){ const s = Math.max(0, Number(m[1]) - 1), e = Math.min(chapA.length-1, Number(m[2]) - 1); for(let i=s;i<=e;i++) renderCombined(i, chapA, chapB); showReadNav(true, s); return; }
        if(/^\d+$/.test(String(state.verseKey))){ const idx = Math.max(0, Math.min(chapA.length-1, Number(state.verseKey)-1)); renderCombined(idx, chapA, chapB); showReadNav(true, idx); return; }
      ...
      console.error('[BibleReader] renderRead failed', err);
      readVerses.innerHTML = '<div style="padding:12px;color:#666">Unable to render chapter</div>';
    }
  }
  ...
  function setVerseByIndex(idx){ const n = normCache[state.versionA]; if(!n) return; const ch = n.books[state.bookIndex].chapters[state.chapterIndex]; if(!ch || idx < 0 || idx >= ch.length) return; state.verseKey = ch[idx].key; renderRead(); updateUrl('push'); }
  ...
  playBtn && playBtn.addEventListener('click', ()=> { try{ speechSynthesis.cancel(); buildTTS(); speakNext(); }catch(e){} });
  ...
  async function doSearch(q){
    ...
    searchResults.appendChild(frag); showView('search');
  }

  document.addEventListener('DOMContentLoaded', ()=> {
    if(searchBox) searchBox.onkeydown = (e) => { if(e.key === 'Enter'){ const q = searchBox.value.trim().toLowerCase(); doSearch(q); } };
  });
  ...
  (function attachSwipe(){
    ...
    readVerses.addEventListener('touchend', e => {
      const dx = e.changedTouches[0].clientX - startX; if(Math.abs(dx) < 60) return;
      const n = normCache[state.versionA]; if(!n) return;
    ...
    let mouseDown = false, msStart = 0, msCur = 0;
    readVerses.addEventListener('mousedown', e => { mouseDown = true; msStart = e.clientX; });
    document.addEventListener('mousemove', e => { if(!mouseDown) return; msCur = e.clientX; });
    document.addEventListener('mouseup', e => {
      if(!mouseDown) return; mouseDown = false;
      const dx = (msCur || e.clientX) - msStart; if(Math.abs(dx) < 100) { msStart = msCur = 0; return; }
      const n = normCache[state.versionA]; if(!n) return;
      if(dx < 0){ if(state.chapterIndex + 1 < n.books[state.bookIndex].chapters.length){ state.chapterIndex++; state.verseKey=null; renderRead(); updateUrl('push'); } }
      else { if(state.chapterIndex > 0){ state.chapterIndex--; state.verseKey=null; renderRead(); updateUrl('push'); } }
      msStart = msCur = 0;
  ...
  async function initialLoad(){
    try{
      ...
      console.error('[BibleReader] initialLoad failed', err);
    }
  }
